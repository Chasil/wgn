...
$sql = "SELECT NULL AS id, IF( m.nazwa = p.nazwa, 1, 0 ) AS isMain, m.nazwa AS city, p.nazwa AS district, w.nazwa AS province, NULL AS lat, NULL AS lng, IF( m.nazwa = p.nazwa, CONCAT_WS(  ', ', w.nazwa, m.nazwa, doe.dzielnica ) , CONCAT_WS(  ', ', w.nazwa, p.nazwa, m.nazwa, doe.dzielnica ) ) AS name, doe.dzielnica AS section, NULL AS subregion, NULL AS street
FROM miasto m
JOIN powiat p ON m.id_pow = p.id
JOIN wojewodztwo w ON m.id_woj = w.id
JOIN dzielnicaosiedle doe ON doe.id_miasta = m.id
GROUP BY w.nazwa, p.nazwa, m.nazwa, doe.dzielnica";
...
$statement2 = $db->prepare('INSERT INTO location_autocomplete (name,province,district,city,section,isMain) VALUES(:name,:province,:district,:city,:section,:isMain)');
