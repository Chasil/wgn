...
class LinkManager {
    ...
    public function getPublishedByCategoryId($idCategory,$limit=10) {
        $repo = $this->services->get('doctrine')->getManager()
                     ->getRepository('AppSearchLinkBundle:Link');
    ...
    public function getAll(){
        ...
        $repo = $this->services->get('doctrine')->getRepository('AppSearchLinkBundle:Link');
       ...
       if(!$request->query->has('idCategory')){
           $category = $this->services->get('search_link_category.manager')->getFirst();
