 ...
 */
class ImportFromArchiveCommand extends ContainerAwareCommand
{
    ...
    public function __construct() {
        ...
        $this->path = __DIR__ .'/../../../../../inet/archiwum/';
        $this->counter = 0;
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->logger = $this->getContainer()->get('monolog.logger.archive');
        $this->output = $output;
        ...
        try{
            if ($zip->open($this->path.$input->getArgument('file')) !== TRUE) {
                throw new \Exception('plik nie został rozpakowany. ' . $this->path.$input->getArgument('file'));
            }
            ...
            //$now = new \DateTime();
            //$newName = $now->format('Y-m-d_H').$input->getArgument('file');

            //$this->fs->rename($this->path.$input->getArgument('file'), $this->path.$newName);
    ...
    protected function importAction($record){
        ...
        $this->logger->info(sprintf('Zaimportowano wszystkich: %s',$this->counter));
    }
    ...
    protected function add($record){
        $offer = $this->getOffer($this->getImportId($record));
        ...
        if($offer){
            ...
            $this->counter++;
            $this->output->writeln($this->getImportId($record));
            $this->output->writeln(sprintf('zaimportowano: %s',$this->counter));
    ...
    protected function prepareData($record){
        ...
        $name = $this->prepareName(array('idCategory'=>$idCategory,
        ...
        echo $importId = $this->getImportId($record);
        $expire = clone $now;
...
//        return $name;
    ...
    private function getType($kind,$type, $propertyType, $location){
        ...
        $types[2]['rezydencjalny'] = array('category'=>2,'type'=>17);
