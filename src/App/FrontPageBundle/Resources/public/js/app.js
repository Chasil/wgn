...
$('#advanced-search,#advanced-search, #advanced-search-second').on('click',function(e){
    e.preventDefault();
    var self = this;
    if($(self).hasClass('more')){
        $('#search-advanced-options').slideDown('slow',function(){
            $('#advanced-search, #advanced-search-second').removeClass('more');
            $('#search-options-second').css('display','block');
            $('#advanced-search, #advanced-search-second').addClass('less');
            $('#advanced-search, #advanced-search-second').html('zwiń opcje');

        });

    }else {
        $('#search-advanced-options').slideUp('slow',function(){
            $('#advanced-search, #advanced-search-second').addClass('more');
            $('#search-options-second').css('display','none');
            $('#advanced-search, #advanced-search-second').removeClass('less');
            $('#advanced-search, #advanced-search-second').html('rozwiń opcje');
        });
    }
});
...
$('select[name="search\\[transactionType\\]"], select[name="search\\[category\\]"]').on('change',function(){
    loadCountries('select[name="search\\[country\\]"]',$('select[name="search\\[category\\]"]').val(),$('select[name="search\\[transactionType\\]"]').val());
});
...
$('.mobile-corp-finder h5').on('click',function(e){
    console.log('test00');
    e.preventDefault();
    if($(this).parent('.mobile-corp-finder').find('.offices-list').is(':visible')){
        $(this).parent('.mobile-corp-finder').find('.offices-list').hide();
    }else {
        $(this).parent('.mobile-corp-finder').find('.offices-list').show();
    }
...
//       });
// };
$( "input[name='search\\[locationIndexLike\\]']" ).on( "keyup", function() {

   if($(this).val().length > 1){
     searchLocation($(this).val());
   }
});
$( "input[name='search\\[locationIndexLike\\]']" ).on( "focus", function() {
          var content = '';
...
$('.wgn-main-find-btn').on('click',function(){
    ...
    var query = $(this).closest('form').find("input[name='search\\[locationIndexLike\\]']").val();
    ...
    if(query==='Cała Polska'){
        $("input[name='search\\[locationIndexLike\\]']").val('');
    }
});
$("input[name='search\\[locationIndexLike\\]']" ).on( "focusout", function() {
    ...
    $('#search-mobile-box-form, #search-box-form').on('click','.search-result-row',function(){
        $("input[name='search\\[locationIndexLike\\]']").val($(this).data('name'));
        $('.l-suggestions').css('display','none');
    });
...
$('#mm-left a').on('click',function (e) {
    e.preventDefault();
    if($('#mm-content').hasClass('hidden')){
        $('#mm-content').removeClass('hidden');
    }else {
        $('#mm-content').addClass('hidden');
    }

});
$('#mm-search-button a').on('click',function (e) {
    e.preventDefault();
    if($('#search-mobile-box').hasClass('hidden')){
        $('#search-mobile-box').removeClass('hidden');
    }else {
        $('#search-mobile-box').addClass('hidden');
    }

});
$('#mobile-contact .message').on('click',function (e) {
    e.preventDefault();
    $('html, body').animate({
...
$('#field-filters input').on('click', function (e) {
    e.preventDefault();
    $('#row-bottom-mobile').toggle();
})
